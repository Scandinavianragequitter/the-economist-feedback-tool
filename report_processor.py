...
def parse_and_enrich_report(raw_text: str) -> List[Dict[str, Any]]:
    # Handle lazy LLM numbered lists
    ...
    for p in paragraphs:
        ...
        # Clean text by removing all citation brackets
        full_line = re.sub(r"\[\[.*?\]\]", "", p).strip()
        
        # --- TOPIC EXTRACTION LOGIC ---
        topic, insight = "General", full_line
        if ":" in full_line[:30]: # Look for the first colon within the first 30 chars
...
def main():
    ...
    print(f"✅ Success: Topic-aware report saved to {OUTPUT_FILENAME}")
